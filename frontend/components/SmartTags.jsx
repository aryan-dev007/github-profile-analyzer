...
const TONE_CLASSES = {
  ...
  alert: "bg-rose-400/15 text-rose-100"
};
  ...
  return (
    <div className="glass-card glass-strong neon-border card-hover p-6 fade-up">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">Smart Tags</h2>
        ...
        {languageBreakdown.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {languageBreakdown.slice(0, 3).map((lang) => (
              <span
                key={lang.name}
                className="rounded-full border border-slate-700/50 bg-white/5 px-2 py-1 text-slate-200"
              >
