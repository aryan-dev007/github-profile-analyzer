...
const TONE_CLASSES = {
  good: "bg-emerald-100 text-emerald-900 dark:bg-emerald-500/20 dark:text-emerald-100",
  warn: "bg-amber-100 text-amber-900 dark:bg-amber-500/20 dark:text-amber-100",
  alert: "bg-rose-100 text-rose-900 dark:bg-rose-500/20 dark:text-rose-100"
};
  ...
  return (
    <div className="glass-card p-6 fade-up">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold">Smart Tags</h2>
        ...
        {languageBreakdown.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {languageBreakdown.slice(0, 3).map((lang) => (
              <span
                key={lang.name}
                className="rounded-full border border-slate-200 bg-white/80 px-2 py-1 text-slate-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200"
              >
